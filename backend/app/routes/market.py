...
from app.config import SMARTAPI_EXCHANGE, CACHE_TTL_CANDLES_SECONDS, CACHE_TTL_SNAPSHOT_SECONDS
...
async def get_snapshot(symbol: str = Query(..., description="e.g. RELIANCE")):
    ...
    try:
        ...
        await set_cache(key, out, ttl=CACHE_TTL_SNAPSHOT_SECONDS)
    ...
    if db_snap:
        await set_cache(key, db_snap, ttl=CACHE_TTL_SNAPSHOT_SECONDS)
...
async def get_history(
    ...
    if len(db_candles) >= limit * 0.8 and not db_is_stale:
        out = {"symbol": symbol, "interval": interval, "data": db_candles, "source": "CACHE", "data_source": "CACHE", "count": len(db_candles)}
        await set_cache(key, out, ttl=CACHE_TTL_CANDLES_SECONDS)
    ...
    try:
        ...
        if rows:
            ...
            if ohlcv:
                ...
                out = {"symbol": symbol, "interval": interval, "data": ohlcv, "source": "NSE_API", "data_source": "NSE_API", "count": len(ohlcv)}
                await set_cache(key, out, ttl=CACHE_TTL_CANDLES_SECONDS)
    ...
    out = {"symbol": symbol, "interval": interval, "data": ohlcv, "source": "MOCK", "data_source": "MOCK", "count": len(ohlcv)}
