...
from app.inference.candle_store import get_candles, get_last_candle, store_candles
from stockai_shared.db.db import PredictionModel, async_session, is_transient_db_error
from app.inference.native_accelerators import compute_feature_frame, compute_indicator_frame
...
from stockai_shared.services.market_state import get_market_status as _raw_market_status
...
# Feature Cache & Bounded Cache definitions
import threading

class FeatureCache:
    """Bounded, thread-safe memory cache for feature dataframes with TTL expiration and LRU eviction."""
    ...
    def get(self, key: str) -> Any:
        ...
        with self._lock:
            if key in self._data:
                ...
                if now - ts < self.ttl:
                    # Refresh LRU (move key to end)
                    self._data.pop(key)
                    self._data[key] = (ts, val)
                    try:
                        from stockai_shared.metrics.metrics import FEATURE_CACHE_HITS
                    ...
                    self._data.pop(key)
            try:
                from stockai_shared.metrics.metrics import FEATURE_CACHE_MISSES
    ...
    def set(self, key: str, val: Any) -> None:
        ...
        with self._lock:
            ...
            try:
                from stockai_shared.metrics.metrics import FEATURE_CACHE_SIZE
                FEATURE_CACHE_SIZE.set(len(self._data))
            except Exception:
                pass
...
def _calculate_candle_rollover_ttl(candle_time_str: str, interval: str) -> int:
    try:
        candle_dt = datetime.strptime(str(candle_time_str).strip(), "%Y-%m-%d %H:%M:%S")
    except Exception:
        return 60

    unit = str(interval)[-1].lower()
    try:
        val = int(interval[:-1])
    except ValueError:
        val = 1
    ...
    if unit == 'm':
        ...
        interval_seconds = val * 86400
    else:
        interval_seconds = 60

    rollover_dt = candle_dt + timedelta(seconds=interval_seconds)

    try:
        from datetime import timezone as dt_timezone
        utc_now = datetime.now(dt_timezone.utc)
        ist_now = utc_now.astimezone(dt_timezone(timedelta(hours=5, minutes=30)))
        ist_naive = ist_now.replace(tzinfo=None)
    except Exception:
        ist_naive = datetime.now()

    remaining = int((rollover_dt - ist_naive).total_seconds())
    return max(10, min(remaining, interval_seconds))


_feature_cache = FeatureCache(maxsize=500, ttl=60.0)
...
async def get_indicators(
    ...
    # Technical Indicator Cache lookup
    last_closed_candle = str(candles[-1].get("time"))
    ind_cache_key = f"ind:v4:{normalized_symbol}:{interval}:{last_closed_candle}"
    cached_ind = await get_cache(ind_cache_key)
    if isinstance(cached_ind, dict):
        try:
            from stockai_shared.metrics.metrics import INDICATOR_CACHE_HITS
            INDICATOR_CACHE_HITS.inc()
        except Exception:
            pass
        return cached_ind

    try:
        from stockai_shared.metrics.metrics import INDICATOR_CACHE_MISSES
        INDICATOR_CACHE_MISSES.inc()
    except Exception:
        pass
    ...
    # Save to technical indicator cache
    ttl = _calculate_candle_rollover_ttl(last_closed_candle, interval)
    await set_cache(ind_cache_key, result, ttl=ttl)
...
async def get_prediction(
    ...
    if isinstance(cached, dict):
        try:
            from stockai_shared.metrics.metrics import PREDICTION_CACHE_HITS
    ...
    try:
        from stockai_shared.metrics.metrics import PREDICTION_CACHE_MISSES
    ...
    feat_cache_key = f"{normalized_symbol}:1m:{last_candle_time}"
    feature_df = _feature_cache.get(feat_cache_key)
    if feature_df is not None:
        logger.debug("[BUNDLE] Feature cache hit for %s", feat_cache_key)

    if feature_df is None:
        try:
            feature_df = await asyncio.to_thread(compute_feature_frame, candles)
            if feature_df is not None:
                _feature_cache.set(feat_cache_key, feature_df)
        except Exception as exc:
