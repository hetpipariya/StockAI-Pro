...
from ..inference.feature_engineering import FEATURE_COLUMNS, compute_features
...
class BacktestRequest(BaseModel):
    symbol: str = Field(..., min_length=1, max_length=30, pattern=r"^[A-Za-z0-9_-]+$", description="Stock Symbol")
...
@router.post("")
async def run_backtest(req: BacktestRequest, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail="Training data not found. Please run build_dataset.py in experiments.",
        )
...
def _load_prediction_pipeline():
    ...
    from ..inference import models as inference_models
    ...
    if model is None or scaler is None or not features_list:
        raise RuntimeError(
            "Model artifacts unavailable. Train or mount model.pkl first."
        )
...
def _execute_backtest_sync(
    target_symbol: str, start_date: str, end_date: str, initial_capital: float
) -> dict:
    ...
    df_test["ml_pred"] = np.where(
        prob_up >= 0.60, 1, np.where(prob_down >= 0.60, 0, -1)
    )
    ...
    for idx, row in df_test.iterrows():
        ...
        if in_pos:
            ...
            if exit_flag:
                ...
                equity_curve.append({"time": t_str, "value": round(capital, 2)})
                in_pos = False
    ...
    if equity_curve[-1]["time"] != str(df_test.iloc[-1]["date"]):
        equity_curve.append(
            {"time": str(df_test.iloc[-1]["date"]), "value": round(capital, 2)}
        )
