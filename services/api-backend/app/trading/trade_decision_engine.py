...
from app.inference.native_accelerators import compute_signal_filter
from stockai_shared.cache.redis_client import get_cache
...
async def get_bundle(
    ...
    key = f"bundle:v4:{symbol.upper()}:{interval}:{limit}:{horizon}"
    try:
        cached = await get_cache(key)
        if isinstance(cached, dict):
            return cached
    except Exception as exc:
        logger.debug("[DECISION] Failed to load bundle from Redis: %s", exc)
    return {}

async def get_ws_state_from_redis() -> str:
    try:
        status = await get_cache("stockai:latest:status:GLOBAL")
...
async def get_last_known_price_from_redis(symbol: str) -> float:
    try:
        tick = await get_cache(f"stockai:latest:tick:{symbol.upper()}")
        if isinstance(tick, dict):
            return float(tick.get("ltp") or tick.get("price") or 0.0)
    except Exception:
        pass
    return 0.0

async def get_last_tick_age_from_redis(symbol: str) -> float:
    try:
        tick = await get_cache(f"stockai:latest:tick:{symbol.upper()}")
        if isinstance(tick, dict) and tick.get("timestamp"):
...
async def evaluate_trade_decision(
    ...
    ws_state = await get_ws_state_from_redis()
    ws_price = await get_last_known_price_from_redis(normalized_symbol)
    ws_tick_age = await get_last_tick_age_from_redis(normalized_symbol)

    market_data = _derive_market_data(
        normalized_symbol,
        bundle,
        ws_state=ws_state,
        ws_price=ws_price,
        ws_tick_age=ws_tick_age,
    )
