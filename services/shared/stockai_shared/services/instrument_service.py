...
from stockai_shared.db.db import AsyncSessionLocal, InstrumentModel, sync_session_factory
from stockai_shared.cache.redis_client import get_cache_sync, set_cache
        ...
        try:
            ...
            try:
                rows = session.query(
                    ...
                    InstrumentModel.token,
                    InstrumentModel.exchange,
                    InstrumentModel.tradingsymbol,
                    InstrumentModel.name,
                    InstrumentModel.instrument_type,
                    InstrumentModel.expiry,
                    InstrumentModel.strike,
                    InstrumentModel.lot_size,
                    InstrumentModel.tick_size,
                    InstrumentModel.isin
                ).all()
            finally:
                session.close()
            ...
            records = [
                InstrumentRecord(
                    ...
                    tradingsymbol=str(row[3] or row[0] or "").strip().upper(),
                    name=str(row[4] or row[0] or "").strip(),
                    instrument_type=str(row[5] or "").strip().upper(),
                    expiry=str(row[6] or "").strip(),
                    strike=row[7],
                    lot_size=max(1, int(row[8] or 1)),
                    tick_size=row[9],
                    isin=str(row[10] or "").strip(),
                )
                for row in rows
    ...
    def search_symbols(
        ...
        for _, info in items:
            ...
            if symbol == lookup:
                ...
                score = 100.0 + min(len(symbol) - len(lookup), 20) * -0.5
            elif tradingsymbol.startswith(lookup):
                score = 90.0
            elif lookup in symbol:
                score = 60.0
            elif lookup in tradingsymbol:
                score = 55.0
            elif lookup in name:
                score = 30.0

            # Direct sequence matcher fallback for fuzzy typos
            if score <= 0:
                similarity = SequenceMatcher(None, lookup, symbol).ratio()
                if similarity >= 0.70:
        ...
        return [dict(info) for _, info in ranked[:capped_limit]]
    ...
    def get_all_symbols(self, exchange: str = "NSE") -> list[str]:
        ...
        with self._lock:
            symbols = [
                ...
                if record.exchange == normalized_exchange
            ]
...
_service_instance: Optional[InstrumentService] = None
...
def get_instrument_service() -> InstrumentService:
    ...
    if _service_instance is None:
        with _service_lock:
            if _service_instance is None:
                _service_instance = InstrumentService()
    return _service_instance


def get_token_by_symbol(symbol: str, exchange: str = "NSE") -> str:
    return get_instrument_service().get_token_by_symbol(symbol, exchange)


def normalize_symbol_input(symbol: str) -> str:
    return InstrumentService.normalize_symbol_input(symbol)


def resolve_symbol_input(symbol: str, exchange: Optional[str] = None) -> tuple[str, str]:
    return InstrumentService.resolve_symbol_input(symbol, exchange)


def get_symbol_by_token(token: str, exchange: Optional[str] = None) -> str:
    return get_instrument_service().get_symbol_by_token(token, exchange)


def get_tradingsymbol(symbol: str, exchange: str = "NSE") -> str:
    return get_instrument_service().get_tradingsymbol(symbol, exchange)
...
def search_symbols(
    ...
    return get_instrument_service().search_symbols(query, limit, exchange)
...
def suggest_symbols(
    ...
    return get_instrument_service().suggest_symbols(prefix, limit, exchange)


def get_all_symbols(exchange: str = "NSE") -> list[str]:
    return get_instrument_service().get_all_symbols(exchange)


def get_instrument_count() -> int:
    return get_instrument_service().get_instrument_count()


def get_last_refresh_at() -> Optional[str]:
    return get_instrument_service().get_last_refresh_at()


async def refresh_instruments_daily(force: bool = False) -> int:
    return await get_instrument_service().refresh_instruments_daily(force=force)


def get_bootstrap_counts() -> dict[str, int]:
    service = get_instrument_service()
    service._ensure_loaded_sync()
    with service._lock:
        records = list(service._records)
    total = len(service._symbol_to_token)
    nse_eq = sum(
        1
        for r in records
        if r.exchange == "NSE" and r.instrument_type in {"", "EQ", "EQUITY"}
    )
    return {
        "total": total,
        "nse_eq": nse_eq,
