...
from stockai_shared.cache.redis_client import get_cache, get_redis, set_cache
...
async def _relay_realtime_message(kind: str, payload: dict[str, Any]) -> None:
    ...
    try:
        from .handler import update_last_live_tick_time
        update_last_live_tick_time()
    except ImportError:
        pass
...
async def get_global_active_subscriptions() -> set[str]:
    ...
    try:
        ...
        return {x.decode("utf-8") if isinstance(x, bytes) else str(x) for x in active}
    except Exception as exc:
        logger.warning("[WS] Failed to get global active subscriptions: %s", exc)
        return set()
...
async def stop_realtime_relay_listener() -> None:
    ...
    if redis_client is not None:
        with suppress(Exception):
            await redis_client.delete(f"stockai:active_subscriptions:{_RELAY_INSTANCE_ID}")
...
class SocketManager:
    ...
    async def _recent_prune_loop(self) -> None:
        """Background task to periodically prune expired recent disconnects and refresh Redis active subscription TTL."""
        logger.info("[WS] Starting background disconnect grace list pruning loop.")
        try:
            while True:
                await asyncio.sleep(30.0)
                now = self._now_monotonic()
                await self._prune_recent_disconnects(now)
                # Refresh the active subscription TTL as well!
                try:
                    from .handler import _update_redis_active_subscriptions
                    await _update_redis_active_subscriptions()
                except Exception as exc:
                    logger.debug("[WS] Background active subscriptions refresh failed: %s", exc)
        except asyncio.CancelledError:
            logger.info("[WS] Background disconnect grace list pruning loop cancelled.")
        except Exception as exc:
            logger.error("[WS] Background disconnect grace list pruning loop error: %s", exc, exc_info=True)
    ...
    async def connect(self, websocket: WebSocket, user_id: int) -> str:
        ...
        if self._prune_task is None or self._prune_task.done():
            try:
                self._prune_task = asyncio.create_task(self._recent_prune_loop())
            except Exception as e:
                logger.warning("[WS] Could not start disconnect pruning loop: %s", e)
            ...
            if restored_symbols:
                async with self._sub_lock:
                    ...
                    for symbol in restored_symbols:
                        ...
                        try:
                            from stockai_shared.metrics.metrics import WS_ROOM_SUBSCRIBERS
                            WS_ROOM_SUBSCRIBERS.labels(symbol=symbol).set(len(self._rooms[symbol]))
                        except Exception:
                            pass
        ...
        try:
            from stockai_shared.metrics.metrics import WS_CONNECTIONS
            WS_CONNECTIONS.inc()
        except Exception:
            pass
        ...
        async with self._sub_lock:
            ...
            for symbol in symbols:
                ...
                if not room:
                    ...
                    try:
                        from stockai_shared.metrics.metrics import WS_ROOM_SUBSCRIBERS
                        WS_ROOM_SUBSCRIBERS.labels(symbol=symbol).set(0)
                    except Exception:
                        pass
                    ...
                    try:
                        from stockai_shared.metrics.metrics import WS_ROOM_SUBSCRIBERS
                        WS_ROOM_SUBSCRIBERS.labels(symbol=symbol).set(len(room))
                    except Exception:
                        pass
