...
from app.inference.models import (ModelEnsemble, ensure_models_loaded,
                                  load_models)
from app.inference.drift_detector import DriftDetector

logger = logging.getLogger(__name__)

# Initialize drift detector and models once at import
...
_last_predictions: LRUCache = LRUCache(maxsize=2000)
...
def predict_symbol(
    ...
    if features_df is not None and len(features_df) > 0:
        ...
        # Track drift on live features
        try:
            drift_detector.add_sample(symbol, features_df.iloc[-1].to_dict())
        except Exception as drift_exc:
            logger.warning("[RUNNER] Drift detector update failed: %s", drift_exc)
    else:
        seq = np.zeros((20, 10))
        tab = np.zeros((1, 10))
