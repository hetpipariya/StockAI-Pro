...
def upgrade() -> None:
    ...
    op.alter_column("orders", "confidence", existing_type=sa.INTEGER(), nullable=True)
    op.alter_column(
        "orders",
        "reason",
    ...
    if bind.dialect.name == "postgresql":
        ...
        op.execute(
            sa.text(
                "UPDATE positions SET id = nextval('positions_id_seq') WHERE id IS NULL"
            )
        )
        op.execute(
            sa.text(
    ...
    op.alter_column("positions", "id", existing_type=sa.Integer(), nullable=False)
    op.add_column("positions", sa.Column("user_id", sa.Integer(), nullable=True))
    if not _column_exists(bind, "positions", "mode"):
        op.add_column(
            "positions", sa.Column("mode", sa.String(length=10), nullable=True)
        )
    op.execute(sa.text("UPDATE positions SET mode = 'paper' WHERE mode IS NULL"))
    op.alter_column(
        "positions", "mode", existing_type=sa.String(length=10), nullable=False
    )
    ...
    if _column_exists(bind, "positions", "timestamp"):
        ...
        op.execute(
            sa.text(
                "UPDATE positions SET opened_at = COALESCE(opened_at, CURRENT_TIMESTAMP) WHERE opened_at IS NULL"
            )
        )
    op.alter_column(
        "positions", "opened_at", existing_type=sa.DateTime(), nullable=False
    )
    op.alter_column("positions", "stop_loss", existing_type=sa.FLOAT(), nullable=True)
    op.alter_column("positions", "target", existing_type=sa.FLOAT(), nullable=True)
    ...
    op.create_unique_constraint(
        "uq_position_user_symbol", "positions", ["user_id", "symbol"]
    )
    op.create_foreign_key(
        None, "positions", "users", ["user_id"], ["id"], ondelete="CASCADE"
    )
    op.drop_column("positions", "timestamp")
    op.add_column("predictions", sa.Column("user_id", sa.Integer(), nullable=True))
    op.alter_column(
        "predictions", "horizon", existing_type=sa.VARCHAR(length=10), nullable=True
    )
    op.alter_column(
        "predictions", "predicted_price", existing_type=sa.FLOAT(), nullable=True
    )
    op.alter_column(
        "predictions", "confidence", existing_type=sa.INTEGER(), nullable=True
    )
    op.alter_column(
        "predictions",
        "explanation",
    ...
    op.create_foreign_key(
        None, "predictions", "users", ["user_id"], ["id"], ondelete="SET NULL"
    )
    ...
    op.alter_column(
        "trade_logs", "direction", existing_type=sa.VARCHAR(length=10), nullable=True
    )
    op.alter_column("trade_logs", "quantity", existing_type=sa.INTEGER(), nullable=True)
    op.alter_column("trade_logs", "price", existing_type=sa.FLOAT(), nullable=True)
    op.alter_column(
        "trade_logs", "confidence", existing_type=sa.INTEGER(), nullable=True
    )
    op.alter_column(
        "trade_logs",
        "reason",
    ...
    op.alter_column(
        "trade_logs", "mode", existing_type=sa.VARCHAR(length=10), nullable=True
    )
    op.alter_column(
        "trade_logs",
        "error",
    ...
    op.create_index(
        "ix_trade_logs_user_symbol", "trade_logs", ["user_id", "symbol"], unique=False
    )
    op.create_index(
    ...
    op.create_foreign_key(
        None, "trade_logs", "users", ["user_id"], ["id"], ondelete="CASCADE"
    )
    ...
    op.execute(
        sa.text(
            "UPDATE users SET starting_capital = 100000.0 WHERE starting_capital IS NULL"
        )
    )
    op.alter_column(
        "users", "starting_capital", existing_type=sa.Float(), nullable=False
    )

    if not _column_exists(bind, "users", "trading_mode"):
        op.add_column(
            "users", sa.Column("trading_mode", sa.String(length=10), nullable=True)
        )
    op.execute(
        sa.text("UPDATE users SET trading_mode = 'PAPER' WHERE trading_mode IS NULL")
    )
    op.alter_column(
        "users", "trading_mode", existing_type=sa.String(length=10), nullable=False
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "trade_logs", "mode", existing_type=sa.VARCHAR(length=10), nullable=False
    )
    op.alter_column(
        "trade_logs",
        "reason",
    ...
    op.alter_column(
        "trade_logs", "confidence", existing_type=sa.INTEGER(), nullable=False
    )
    op.alter_column("trade_logs", "price", existing_type=sa.FLOAT(), nullable=False)
    op.alter_column(
        "trade_logs", "quantity", existing_type=sa.INTEGER(), nullable=False
    )
    op.alter_column(
        "trade_logs", "direction", existing_type=sa.VARCHAR(length=10), nullable=False
    )
    op.alter_column(
        "trade_logs",
        "event",
    ...
    op.alter_column(
        "predictions", "confidence", existing_type=sa.INTEGER(), nullable=False
    )
    op.alter_column(
        "predictions", "predicted_price", existing_type=sa.FLOAT(), nullable=False
    )
    op.alter_column(
        "predictions", "horizon", existing_type=sa.VARCHAR(length=10), nullable=False
    )
    ...
    op.alter_column(
        "positions", "timestamp", existing_type=sa.DateTime(), nullable=False
    )
    ...
    op.alter_column("positions", "target", existing_type=sa.FLOAT(), nullable=False)
    op.alter_column("positions", "stop_loss", existing_type=sa.FLOAT(), nullable=False)
    ...
    op.alter_column("orders", "confidence", existing_type=sa.INTEGER(), nullable=False)
    op.alter_column(
        "orders",
        "order_id",
        existing_type=sa.String(length=100),
