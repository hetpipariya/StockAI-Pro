...
Usage:
    python -m app.inference.data_pipeline                      # full watchlist
    python -m app.inference.data_pipeline --ticker INFY.NS     # single ticker
    python -m app.inference.data_pipeline --validate           # validate tickers only
...
from stockai_shared.services.ticker_map import TICKERS
...
# ── config ────────────────────────────────────────────────────────────────────
BASE_DIR = Path(__file__).resolve().parents[1]  # services/ai-engine/app/
